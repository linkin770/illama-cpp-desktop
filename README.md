...
- **本地知识库系统**：纯本地实现的文档管理和全文检索功能，无需外部依赖
- **多格式文档支持**：支持 PDF、Word (.doc/.docx)、Excel (.xls/.xlsx/.xlsb)、TXT、MD 等格式
- **智能文本分块**：自动按段落和句子切分文档，500 字符/块，50 字符重叠确保上下文连贯
- **TF-IDF 全文检索**：基于词频-逆文档频率算法，支持中英文分词，返回最相关的 Top-3 文本块
- **对话自动注入**：发送消息时自动检索知识库，将相关内容注入 system prompt，模型回答更准确
- **知识库管理面板**：可视化上传、删除、查看文档状态，实时显示解析进度
- **异步解析机制**：上传后立即返回，后台自动解析和分块，不阻塞 UI
- **智能状态轮询**：检测到处理中文档时自动 2 秒刷新，解析完成自动更新状态

#### 🎨 UI 优化

- **知识库入口按钮**：侧边栏底部添加 DatabaseOutlined 图标的知识库按钮
- **文档列表展示**：显示文件名、大小、上传时间、文本块数量、解析状态
- **状态指示器**：处理中 (🔄)、就绪 (✅)、错误 (❌)、等待中 (⏳) 四种状态图标
- **返回聊天按钮**：知识库面板顶部一键返回聊天界面
- **空状态提示**：无文档时显示友好的引导提示

#### 🔧 技术实现

- **纯本地方案**：使用 TF-IDF 算法而非向量数据库，零外部依赖，保护隐私
- **异步处理架构**：上传→复制文件→后台解析→分块→保存索引，全流程异步非阻塞
- **ESM 模块规范**：统一使用 `node:fs` 和 `node:fs/promises` 导入，修复同步/异步方法混用问题
- **CommonJS 兼容**：正确处理 word-extractor 等 CommonJS 模块在 ESM 中的导入
- **类型安全**：完整定义 KnowledgeDocument、KnowledgeChunk、KnowledgeSearchResult 类型
- **状态管理**：useAppState 添加 knowledgeDocuments、knowledgeEnabled、knowledgeLoading 状态

#### 🐛 Bug 修复

- **fs 模块导入错误**：修复 writeFileSync、readFileSync、copyFile、unlink 等函数未导入的问题
- **Word 文档提取失败**：修复 word-extractor API 调用错误，合并 body、headers、footnotes 全部内容
- **文档状态不更新**：添加定时轮询机制，解析完成后自动刷新 UI，无需手动操作
- **CSS 变量未定义**：修复 `var(--accent)` 改为 `var(--green)`，按钮颜色正常显示
- **文件结构损坏**：重建 KnowledgeBasePanel.tsx，修复 return 语句在函数体外的语法错误

#### 💡 用户体验

- **一键上传**：点击上传按钮选择文件，自动解析并显示进度
- **实时反馈**：终端日志详细显示解析过程（文本长度、分块数量等）
- **错误提示**：解析失败时显示具体错误信息，方便排查问题
- **无缝集成**：开启知识库后，聊天时自动检索相关文档，无需额外操作
