...
- **内置聊天界面**: 支持流式回复、历史对话搜索和管理
- **系统托盘后台运行**: 窗口最小化后服务继续在后台运行
- **完整 llama.cpp 集成**: 支持 llama-server 服务管理和配置

### 🎨 用户体验

- **ChatGPT 风格界面**: 纯净白色背景配墨绿点缀
- **代码高亮与预览**: 支持多种编程语言的代码块展示
- **思考过程显示**: 可选显示模型推理过程
- **响应式设计**: 适配不同屏幕尺寸
- **发送按钮状态联动**: 输入内容时变绿，生成时变红可中止

### ⚙️ 高级配置

- **完整参数配置**: ctx\_size、GPU 层数、采样参数等
...
- **Claude Code SKILL.md 兼容**: 支持创建和管理自定义技能提示词
- **LLM 自动生成**: 填写基本信息后由本地模型自动生成完整 SKILL.md
- **技能管理面板**: 在设置中新建、修改、删除技能
- **对话技能选择**: 聊天输入框旁一键选择技能，自动注入系统提示词
...
| Ant Design X     | ^2.7.0  | dependencies    | React 组件库 |
...
- **会话级技能隔离**：技能绑定到当前对话，切换会话后技能自动隐藏，切回后自动恢复
