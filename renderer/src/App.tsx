...
function App() {
 ...
 setModelInfo({ error: error?.message || String(error) });
 }
 ...
 content: result.content || state.chatMessages[lastIndex - 1]?.content || '模型返回了空内容。',
 tokens: displayTokens || estimateTokens(result.content || ''),
 ...
 setChatMessages(state.chatMessages.slice(0, index));
 const requestId = `chat-${Date.now()}-${Math.random().toString(16).slice(2)}`;
 const assistantMessage: ChatMessage = {
 role: 'assistant',
 content: '',
 ...
 updateChatMessage(lastIndex - 1, {
 content: result.content || state.chatMessages[lastIndex - 1]?.content || `基于"${userMessage.content}"重试后，模型返回了空内容。`,
 ...
 }[kind as keyof typeof filters] || [{ name: 'All Files', extensions: ['*'] }];
 ...
 if (payload.requestId !== state.streamRequestId)
 return;
 const lastIndex = state.chatMessages.length - 1;
 const lastMessage = state.chatMessages[lastIndex];
 ...
 if (payload.done) {
 updateChatMessage(lastIndex, {
 content: payload.content || lastMessage.content || '模型返回了空内容。',
 streaming: false,
 });
 setStreamRequestId('');
 saveCurrentSession();
 ...
 window.llamaDesktop?.onEvent?.(handleEvent);
 }, [state.streamRequestId, state.chatMessages, patchFromBackend, updateChatMessage, setStreamRequestId, saveCurrentSession]);
 return (<>
...
<ServiceBar status={state.status} busy={state.busy} dirty={state.dirty} onSave={save} onHealth={health} onStart={start} onStop={stop}/>
 ...
 <SettingsPanel settingsOpen={state.settingsOpen} active={state.active} config={state.config} validation={state.validation} status={state.status} logs={state.logs} dirty={state.dirty} onClose={() => setSettingsOpen(false)} onSelectSection={setActive} onUpdateConfig={updateConfig} onPickFile={pickFile}/>
